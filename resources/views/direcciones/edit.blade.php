<!-- resources/views/direcciones/edit.blade.php -->
            ...
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg p-6">
                <form method="POST" action="{{ route('direcciones.update', $direccion->id) }}">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" id="calle" name="calle" value="{{ $direccion->calle }}" maxlength="255" required class="block w-full bg-white border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500">
                    </div>

                    <!-- Número -->
                    <div class="mb-4">
                        <label for="numero" class="block text-gray-700 text-sm font-bold mb-2">Número</label>
                        <input type="text" id="numero" name="numero" value="{{ $direccion->numero }}" maxlength="20" required class="block w-full bg-white border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500">
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <select name="comuna_id" id="comuna_id" required class="block w-full bg-white border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500">
                            ...
                            @foreach($comunas as $comuna)
                                <option value="{{ $comuna->id }}" {{ $direccion->comuna_id == $comuna->id ? 'selected' : '' }}>{{ $comuna->nombre }}</option>
                            @endforeach
                        </select>
                    </div>
