            ...
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg p-6">
                <form method="POST" action="{{ route('sucursales.update', $sucursal->id) }}">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" id="nombre" name="nombre" value="{{ $sucursal->nombre }}" maxlength="100" required class="block w-full bg-white border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500">
                    </div>

                    <!-- Activa -->
                    <div class="mb-4 flex items-center">
                        <input type="checkbox" id="activa" name="activa" value="1" class="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300 rounded" {{ $sucursal->activa ? 'checked' : '' }}>
                    ...
                    <div class="mb-4">
                        ...
                        <select name="direccion_id" id="direccion_id" class="block w-full bg-white border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500">
                            ...
                            @foreach($direcciones as $direccion)
                                <option value="{{ $direccion->id }}" {{ $sucursal->direccion_id == $direccion->id ? 'selected' : '' }}>{{ $direccion->descripcion }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <select name="sucursal_id" id="sucursal_id" class="block w-full bg-white border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500">
                            ...
                            @foreach($sucursales as $s)
                                <option value="{{ $s->id }}" {{ $sucursal->sucursal_id == $s->id ? 'selected' : '' }}>{{ $s->nombre }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <select name="empresa_id" id="empresa_id" class="block w-full bg-white border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500" required>
                            ...
                            @foreach($empresas as $empresa)
                                <option value="{{ $empresa->id }}" {{ $sucursal->empresa_id == $empresa->id ? 'selected' : '' }}>{{ $empresa->nombre }}</option>
                            @endforeach
                        </select>
                    </div>
